...
describe('save/load roundtrip', () => {
  ...
  it('saveState then loadState roundtrips mutated state', () => {
    const mutated = { ...defaultState, lines: fromNumber(42), money: fromNumber(999), version: CURRENT_SAVE_VERSION };
    saveState(mutated);
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(42));
    expect(loaded.money).toStrictEqual(fromNumber(999));
  ...
  it('partial saved data merges with defaults', () => {
    ...
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(100));
    expect(loaded.money).toStrictEqual(BN_ZERO);
  ...
  it('saved/loaded state preserves current version', () => {
    const mutated = { ...defaultState, lines: fromNumber(50) };
    saveState(mutated);
    const loaded = loadState(defaultState);
    expect(loaded.version).toBe(CURRENT_SAVE_VERSION);
  });
  ...
  it('old save with no version loads as current version', () => {
    ...
    expect(loaded.version).toBe(CURRENT_SAVE_VERSION);
    expect(loaded.lines).toStrictEqual(fromNumber(100));
    expect(loaded.money).toStrictEqual(fromNumber(50));
  });
  ...
  it('numeric field loaded as string falls back to default', () => {
    ...
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(BN_ZERO);
  });
  ...
  it('ascensionMultiplier below 1 falls back to default', () => {
    ...
    const loaded = loadState(defaultState);
    expect(loaded.ascensionMultiplier).toStrictEqual(BN_ONE);
  });
  ...
  it('saveState stamps lastSavedAt with Date.now()', () => {
    ...
    saveState({ ...defaultState, lines: fromNumber(5) });
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(5));
  ...
  it('debouncedSave stamps lastSavedAt with Date.now()', () => {
    vi.useFakeTimers();
    const before = Date.now();
    debouncedSave({ ...defaultState, lines: fromNumber(10) });
    vi.advanceTimersByTime(200);
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(10));
    expect(loaded.lastSavedAt).toBeGreaterThanOrEqual(before);
    vi.useRealTimers();
  });
});
...
describe('save backup', () => {
  it('first save does not create a backup', () => {
    saveState({ ...defaultState, lines: fromNumber(10) });
    const backup = localStorage.getItem('vibe_coder_save_backup');
    expect(backup).toBeNull();
  });

  it('second save backs up previous primary save', () => {
    const first = { ...defaultState, lines: fromNumber(10) };
    saveState(first);
    saveState({ ...defaultState, lines: fromNumber(20) });
    const backup = localStorage.getItem('vibe_coder_save_backup');
    expect(backup).not.toBeNull();
    expect(JSON.parse(backup!).lines).toStrictEqual(fromNumber(10));
  });

  it('later saves update backup to previous primary', () => {
    saveState({ ...defaultState, lines: fromNumber(10) });
    saveState({ ...defaultState, lines: fromNumber(20) });
    saveState({ ...defaultState, lines: fromNumber(30) });
    const backup = localStorage.getItem('vibe_coder_save_backup');
    expect(JSON.parse(backup!).lines).toStrictEqual(fromNumber(20));
  });
  ...
  it('debouncedSave backs up previous primary save', () => {
    ...
    debouncedSave({ ...defaultState, lines: fromNumber(20) });
    vi.advanceTimersByTime(200);
...
describe('backup restore on load', () => {
  it('uses backup when primary is missing', () => {
    ...
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(50));
  });
...
describe('export/import', () => {
  ...
  it('deserializeState roundtrips mutated state', () => {
    const mutated = { ...defaultState, lines: fromNumber(42), money: fromNumber(999) };
    const json = serializeState(mutated);
    const result = deserializeState(json, defaultState);
    expect(result).not.toBeNull();
    expect(result!.lines).toStrictEqual(fromNumber(42));
    expect(result!.money).toStrictEqual(fromNumber(999));
  });
  ...
  it('deserialized state can be saved and loaded', () => {
    const mutated = { ...defaultState, lines: fromNumber(77) };
    const json = serializeState(mutated);
    const result = deserializeState(json, defaultState);
    expect(result).not.toBeNull();
    saveState(result!);
    const loaded = loadState(defaultState);
    expect(loaded.lines).toStrictEqual(fromNumber(77));
  });
});
...
describe('offline progress on load', () => {
  it('lastSavedAt > 0 applies offline gains', () => {
    ...
    expect(loaded.lines).toStrictEqual(expected.lines);
    expect(toNum(loaded.money)).toBeCloseTo(toNum(expected.money), 1);
  });
