...
describe('autoBuyUpgrades', () => {
  ...
  it('buys ED perk tier 1', () => {
    const s = makeState({ edOwned: 3, money: fromNumber(600), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.perkEdTier).toBe(1);
    expect(result.money).toStrictEqual(fromNumber(100));
  });

  it('buys ED perk tier 2', () => {
    const s = makeState({ edOwned: 5, perkEdTier: 1, money: fromNumber(2500), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, buyCheapest: false, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.perkEdTier).toBe(2);
    expect(result.money).toStrictEqual(fromNumber(500));
  });

  it('buys KB perk tier', () => {
    const s = makeState({ kbOwned: 10, money: fromNumber(6000), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.perkKbTier).toBe(1);
    expect(result.money).toStrictEqual(fromNumber(1000));
  });

  it('buys Lint perk tier', () => {
    const s = makeState({ lintOwned: 10, money: fromNumber(6000), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
  ...
  it('buys Energy Flux', () => {
    const s = makeState({ edOwned: 5, money: fromNumber(200), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.fluxOwned).toBe(1);
  });

  it('buys Flux perk tier', () => {
    const s = makeState({ fluxOwned: 25, money: fromNumber(15000), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.perkFluxTier).toBe(1);
    expect(result.money).toStrictEqual(fromNumber(5000));
  });
  ...
  it('skips unmet prerequisites', () => {
    const s = makeState({ edOwned: 2, money: fromNumber(600), autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, moneyReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    expect(result.perkEdTier).toBe(0);
  });

  it('buys exactly one upgrade per call', () => {
    const s = makeState({ edOwned: 3, money: fromNumber(600), vibeLevel: 2, autoUpgrades: { ...defaultState.autoUpgrades, enabled: true, buyCheapest: false, moneyReservePct: 0, vibeReservePct: 0 } });
    const result = autoBuyUpgrades(s);
    let changes = 0;
    if (result.perkEdTier !== s.perkEdTier) changes++;
    if (result.masteryFocusScroll !== s.masteryFocusScroll) changes++;
    if (toNum(result.money) !== toNum(s.money)) changes++;
  ...
  it('no candidates returns unchanged copy', () => {
    const s = makeState({ money: fromNumber(0), vibeLevel: 0, autoUpgrades: { ...defaultState.autoUpgrades, enabled: true } });
    const result = autoBuyUpgrades(s);
